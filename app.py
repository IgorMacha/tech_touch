...
def gerar_passos_plano(model, fatos, gaps, dias, tom, analista="", nao_iniciada=False, data_min=""):
    ...
    user = (
        ...
        f"Estrutura de cada mensagem: saudação breve com o nome do contato quando houver; uma frase de contexto "
        f"dizendo, de forma leve e sem cobrança, que você percebeu que a frota ainda não está usando essa "
        f"ferramenta (que é essencial para a operação) e por isso resolveu chamar; 1 frase do ganho concreto; "
        f"um mini passo a passo de 2 a 3 passos RESUMIDO a partir de 'passos_manual' (não invente telas que não "
        f"estejam ali); e uma frase de convite para fazer junto.\n"
